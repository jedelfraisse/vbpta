    ...
    UserManager<SiteUser> userManager) : IAdminDashboardService
{
    ...
    public async Task<Guid> CreateSiteAsync(string? currentUserId, AdminCreateSiteRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var site = new Site
        {
            ...
            LogoUrl = useDefaultLogo ? "images/logo.png" : GetValueOrDefault(request.LogoUrl, SeedData.DefaultCitySite.LogoUrl),
            BannerUrl = useDefaultBanner ? "images/banner.png" : GetValueOrDefault(request.BannerUrl, SeedData.DefaultCitySite.BannerUrl),
        ...
        await _sitePublicAssetService.EnsureSiteFoldersAsync(
            normalizedPtaId,
            seedDefaults: useDefaultLogo || useDefaultBanner,
            cancellationToken);
