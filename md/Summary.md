...
_Prepared 2026-08-26. Updated 2026-08-28 to reflect the Organization Framework Phase 1 completion — see the "Phase 1 update" callout below and the `**Phase 1**` annotations throughout. Original analysis reflects the codebase as of commit `bc6fc7c` plus the uncommitted OrganizationType/Organizations work in progress at the time._
...
> **Phase 1 update (2026-08-28):** the Organization Framework described as a gap throughout the original analysis below is now built and complete — see [OrganizationFramework.md](OrganizationFramework.md) (architecture), [OrganizationFramework-Phase1.md](OrganizationFramework-Phase1.md) (implementation plan, Status: Complete), and [OrganizationFramework-Phase1-Notes.md](OrganizationFramework-Phase1-Notes.md) (decisions made + validation). In short: `Organization`, `OrganizationLevel`, `OperationalCycle`, and `ParentAccessGrant` now exist as real, EF-mapped, Global-Admin-manageable entities (`OrganizationService`, `/globaladmin/organizations/**`), layered on top of `Site` rather than replacing it. This closes the single biggest gap the original analysis flagged (§2, §12) — the Event Framework now has a real hierarchy/identity layer to build on, not just `Site`'s fixed two-level shape. What Phase 1 explicitly did **not** touch: `SiteUser`/`SiteUserRole`/`CustomRole`/`BoardPosition` (still Site-scoped), `SiteRoleResolver`, and site/hostname resolution — all still exactly as described below. The rest of this document is left as originally written, with `**Phase 1**` notes marking what's since changed.
...
_Original analysis below is superseded on hierarchy/identity by the Organization Framework — see the Phase 1 callout at the top. Kept as-is since it's still accurate for what it describes (Site, roles, memberships); `**Phase 1**` notes mark what's new._
...
### Organizations
- **`Site`** ([SiteEngine/Entities/Site.cs](../SiteEngine/Entities/Site.cs)) is simultaneously "the organization" and "the website" — one row is both a Division/Local Unit's identity record and its hosting/branding config (hostname, domain, theme colors, logos, social links, status). ~~There is no separate "Organization" entity distinct from Site.~~ **Phase 1:** there now is — see below.
- **`OrganizationType`** ([SiteEngine/Entities/OrganizationType.cs](../SiteEngine/Entities/OrganizationType.cs)) is a *taxonomy* row ("PTA", eventually "Boy Scouts" etc.) — it's what PortalHome/`/organizations` render to describe the kinds of groups the portal supports. **Phase 1:** no longer "deliberately minimal" — it now owns its `OrganizationLevel`s and `OperationalCycle`s, and carries an `IdentifierRequirement`/`IdentifierLabel` policy (e.g. PTA's "00000000" PTA ID #, per-type optional/required/not-used). Still no rules/tasks/branding link — that's still real future work — but the "extend this for type-specific behavior" note below is now half-true: levels/cycles/identifiers are covered, event/tool/theme availability per type is not.
- ~~Hierarchy is exactly two levels: `Site.ParentSiteId`... No sub-Division grouping.~~ **Phase 1:** `Site`'s own hierarchy is still exactly two levels (unchanged — Site resolution wasn't touched), but that's no longer the only hierarchy in the system. `Organization`/`OrganizationLevel` (self-referencing `Organization.ParentOrganizationId`, data-driven `OrganizationLevel` with a `Rank`) supports arbitrary depth per Organization Type — validated with real 5-level (PTA), 4-level (Scouts), and 4-level (Billiards) hierarchies. An Organization optionally links to a `Site` (0-or-1, via `Organization.SiteId`) rather than being one. See [OrganizationFramework-Phase1-Notes.md](OrganizationFramework-Phase1-Notes.md).

### Departments / Committees
Do not exist, still — Phase 1 deliberately did not add these; they weren't in scope. The closest analog is `BoardPosition` (a named position like "Treasurer" held by a `SiteUser` at a `Site` for a `SchoolYear`) and free-text `PositionName`/`SiteUserRole.CustomRole.Name` strings — there's no structured "this position belongs to this committee" relationship. If the Event Framework needs committee-scoped events (e.g. a Ways & Means committee running a fundraiser), that's new schema.
...
Per [CLAUDE.md](../CLAUDE.md) section 2, "Groups" (permission bundles like "Event Managers", "Bingo.Caller") are still a **documented intent**, not implemented — Phase 1 did not build this (it's a different concept from `ParentAccessGrant`, see §3 below). There is still no `Group`/`PermissionGroup` entity anywhere in the schema. This remains the single biggest gap between the architecture doc and the actual code, and it's directly relevant to Director/Helper Controls (see §10) since those will likely want exactly this kind of tool-scoped group.

### Memberships
**`SiteUserRole`** ([SiteEngine/Identity/SiteUserRole.cs](../SiteEngine/Identity/SiteUserRole.cs)) is the actual membership join table: `SiteUser` × `Site` × (`SiteRole` or `CustomRole`) × `SchoolYear`, with optional `StartUtc`/`EndUtc` for partial-year terms and an `IsPrimaryContact` flag. A user can hold multiple rows for the same site/year (e.g. both `SuperAdmin` and `SiteAdmin` — `DashboardService.GetMembershipsAsync` explicitly groups these back together for display). This is the row an Event Framework's "who can register/who's a director for this event" logic would join against. **Phase 1 explicitly did not touch this** — membership migration to Organization-scoping is analysis-only (documented in [OrganizationFramework.md](OrganizationFramework.md)'s "Relationship to Today's Membership Schema"), a later phase's real work.

### Organization Framework entities (Phase 1, new)
- **`Organization`** — the community itself: `Name`, `OrganizationTypeId`, `OrganizationLevelId`, self-referencing `ParentOrganizationId`, optional `SiteId`, optional `IdentifierValue`. Always exists once a community is created, regardless of whether it has a hosted `Site`.
- **`OrganizationLevel`** — a position within a Type's hierarchy (`Name`, `Rank`, `IsSiteEligible`) — a classification an Organization references, not a layer it lives under.
- **`OperationalCycle`** — a structured (`StartDate`/`EndDate`/`DisplayLabel`/`CycleTypeName`) operating period scoped to `OrganizationType`, generalizing what `SchoolYear` assumed for everyone (School Year, League Session, Fiscal Year, ...).
- **`ParentAccessGrant`** — the only mechanism for cross-organization access (`View`/`Participation`/`Administrative`, `ParentOrganizationId` × `ChildOrganizationId`). No implicit cascade; absence of a row means no access. Deleting an Organization cleans up its own grants automatically (child Organizations still block deletion deliberately).
- All five managed via `OrganizationService` ([WebApp/Services/OrganizationService.cs](../WebApp/Services/OrganizationService.cs)) and Global Admin → Organizations, following the exact DTO-record/`IDbContextFactory` pattern described in §1.
...
**Phase 1:** a second, genuinely different cross-*organization* (not cross-role) permission mechanism now exists — `ParentAccessGrant` (§2 above). It answers "can Organization A see/administer Organization B," explicit-only, no implicit cascade, at any depth — not "what can a user do." Worth knowing as a working, validated precedent for "explicit-grant, no-implicit-inheritance" permission design, even though it doesn't replace or connect to the still-unbuilt `ToolPermission`/Groups gap described below.
...
### A working admin-UI precedent (Phase 1, new)
Unlike the dormant systems below, Global Admin → Organizations is a fully wired-up example of the "list page + detail page with in-page tabs" pattern: `OrganizationTypeDetail.razor`/`OrganizationDetail.razor` hold client-side tab state (`Description`/`Levels`/`Cycles`; `Description`/`Branding`) and swap content with plain `@onclick` + `@if` blocks — no routing between tabs, no separate pages per concept. Reusable panel components (`OrganizationLevelsPanel`, `OperationalCyclesPanel`) hold the actual CRUD logic so it isn't duplicated between the detail page and (formerly) standalone pages. Worth reusing this shape for any future Event Framework admin UI (e.g. an Event's own Description/Schedule/Registrants tabs) rather than defaulting to one page per concern the way the rest of Global Admin still does.
...
**Phase 1, new:**
- **`OperationalCycle`** is a real, working, structured (start/end date + display label + type) alternative to the `SchoolYear` string above — a better template than `SchoolYear` itself for an Event Framework's own recurring-series concept, since it supports genuinely different cycle shapes (School Year, League Session, Fiscal Year) without code branching, and its dates are queryable, unlike a formatted string.
- **`ParentAccessGrant`** is a real, working example of "explicit cross-boundary access, no implicit cascade, at any depth" — the exact shape a cross-organization Event visibility/sharing feature (e.g. a Division's event visible to its Units) would want, and a concrete precedent for how Director/Helper access might eventually be modeled if it needs to cross Organization boundaries rather than just user-role boundaries.
- **`Organization`/`OrganizationLevel`** is now the answer to "what does an Event belong to" — see §10's updated recommendation below, which corrects the original (pre-Phase-1) advice to hang events off `Site`.
...
**Event Framework (core).** ~~Model it as new `SiteEngine` entities... a `SiteId` FK... Do NOT invent a separate "Organization" concept — reuse `Site`.~~ **Superseded by Phase 1** — that concept now exists, on purpose, for exactly this. Model `Event` with an `OrganizationId` FK, not a `SiteId` FK: an Organization is the real community/identity node (works whether or not it has a hosted `Site`), and it already carries the hierarchy (`ParentOrganizationId`) an Event Framework would want for "does this Division's event roll up to/get inherited by its Units." Use `OperationalCycle` (not a new `SchoolYear`-style string) if Events need cycle scoping. Everything else still holds: a `Guid Id`, its own `EventService` in `WebApp/Services` built on `IDbContextFactory<AppDbContext>` returning DTO records the same way `OrganizationService`/`DashboardService` do — no new architectural layer (repository/CQRS) just for this module.
...
- Event core entities/service (the `Event` itself; per §10's Phase 1 correction, scoped to `Organization`/`OperationalCycle` rather than `Site`/`SchoolYear`, inheriting down `Organization.ParentOrganizationId` rather than Division→Unit specifically).
...
- **Phase 1:** `OrganizationService`'s pattern for a CRUD-heavy admin service with cross-entity validation (hierarchy placement, identifier uniqueness scoped to a parent Type, cascade-safe delete) is now the concrete template for an `EventService` doing the analogous checks (e.g. a Director assignment must reference a valid Event + Organization pair).
...
- ~~**`SiteType` is a strict two-level hierarchy (Portal → Division → LocalUnit) with no room for a third level.**~~ **RESOLVED by Phase 1.** `SiteType`/`Site.ParentSiteId` itself is still exactly two levels (untouched, by design), but `Organization`/`OrganizationLevel` now provides arbitrary-depth hierarchy alongside it — validated with real 4-5 level hierarchies for three different Organization Types. An Event needing a Division-wide, cross-unit grouping now has a real place to hang off (an `Organization` at whatever `OrganizationLevel` fits), without needing a third `SiteType`.
